...
export async function GET() {
  try {
    ...
    const uniqueInteractedUsers = new Set(announcementReads?.map(r => r.user_id) || []).size;
    const totalReads = announcementReads?.filter(r => r.is_read === true).length || 0;
    const totalUnreads = announcementReads?.filter(r => r.is_read === false || r.is_read === null).length || 0;
    const averageReads = totalAnnouncements > 0 ? (totalReads / totalAnnouncements) : 0;
    const overallReadPercentage = totalUsers > 0 ? (totalReads / (totalUsers * totalAnnouncements)) * 100 : 0;

    // Calculate recent announcements with read counts
    const recentAnnouncements = announcements?.slice(0, 10).map(announcement => {
      const reads = announcementReads?.filter(r => r.announcement_id === announcement.id) || [];
    ...
    announcements?.forEach(announcement => {
      const month = new Date(announcement.created_at).toISOString().substring(0, 7); // YYYY-MM format
      const reads = announcementReads?.filter(r => r.announcement_id === announcement.id) || [];
      const readCount = reads.filter(r => r.is_read === true).length;
      const unreadCount = reads.filter(r => r.is_read === false || r.is_read === null).length;
    ...
    for (const [month, data] of monthlyData.entries()) {
      const averageReadRate = (data.totalReads + data.totalUnread) > 0 ? 
        Math.round((data.totalReads / (data.totalReads + data.totalUnread)) * 100) : 0;
